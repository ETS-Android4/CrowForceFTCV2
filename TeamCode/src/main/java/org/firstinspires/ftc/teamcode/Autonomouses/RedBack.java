...
@Autonomous(group = "Comp", name = "red back")
class RedBack extends LinearOpMode {
    @Override
    public void runOpMode() throws InterruptedException {
        ...
        if(opModeIsActive()) {
            ...
            move.tPID(new Pose2D(0, -4, 0));
            waitTime(500);
            turn.turnPID( Math.PI/2);
            waitTime(500);
            move.tPID(new Pose2D(0, 24, 0));
            waitTime(500);
            turn.turnPID(5 * Math.PI/8);
            waitTime(500);
            move.tPID(new Pose2D(0, 10, 0));
            waitTime(500);
            Baguette.duckSpinner.spinMotorForTime(0.35, 3000);
            turn.turnPID(0);
            waitTime(500);
